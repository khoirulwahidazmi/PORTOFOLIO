...
const SkillsPage = () => {
  ...
  const getSkillColor = (category) => {
    const colorMap = {
      professional: 'from-blue-600 to-blue-400',
      technical: 'from-purple-600 to-purple-400',
      technology: 'from-green-600 to-green-400',
      soft: 'from-pink-600 to-pink-400'
    };
    return colorMap[category] || 'from-gray-600 to-gray-400';
  };

  const renderSkillSection = (title, skillList, category) => {
    if (activeCategory !== 'all' && activeCategory !== category) return null;
    
    const Icon = getSkillIcon(category);
    const colorClass = getSkillColor(category);

    return (
      <Card key={category} className="bg-white/10 backdrop-blur-sm border-white/20 p-8 hover:bg-white/15 transition-all duration-300">
        <div className="flex items-center mb-6">
          <div className={`p-3 bg-gradient-to-r ${colorClass} rounded-xl mr-4`}>
  ...
  const renderSoftSkills = () => {
    if (activeCategory !== 'all' && activeCategory !== 'soft') return null;
